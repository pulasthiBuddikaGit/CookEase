      ...
      <View style={styles.container}>
        ...
        <View style={styles.infoContainer}>
          {loading ? <ActivityIndicator size="large" color="#00796b" /> : (
            <>
              <InfoBox title="Height (cm)" value={dietPlan?.height ?? "N/A"} />
              <InfoBox title="Weight (kg)" value={dietPlan?.weight ?? "N/A"} />
        ...
        <View style={styles.calorieBox}>
          <View style={styles.row}>
            ...
            <Text style={styles.infoText2}>
              {loading ? "Loading..." : dietPlan?.total_calories ? `${dietPlan.total_calories} kcal` : "N/A"}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.dietContainer}>
          ...
          {loading ? <ActivityIndicator size="large" color="#00796b" /> : error ? (
            ...
            <>
              ...
              <FlatList
                ...
                renderItem={({ item }) => <Text style={styles.meal}>• {item}</Text>}
...
const styles = StyleSheet.create({
  ...
  infoContainer: { flexDirection: 'row', justifyContent: 'space-between', width: '100%', paddingHorizontal: 10, marginBottom: 20 },
  infoBox: {
    backgroundColor: '#fff', padding: 15, borderRadius: 10, alignItems: 'center', width: '30%',
    elevation: 3, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 4,
  },
  infoTitle: { fontSize: 14.25, fontWeight: 'bold', color: '#000', marginBottom: 8 },
  infoText: { fontSize: 16, color: '#444' },

  // Calorie Box
  calorieBox: { backgroundColor: '#d4edda', padding: 15, borderRadius: 10, marginBottom: 20, width: '95%', elevation: 3 },
  row: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', margin: 12 },
  infoTitle2: { fontSize: 18, fontWeight: 'bold', color: '#000' },
  infoText2: { fontSize: 18, fontWeight: 'bold', color: '#00796b' },

  // Diet Plan Box
  dietContainer: {
    backgroundColor: '#fff', padding: 20, borderRadius: 10, elevation: 3, marginBottom: 20, width: '95%',
  ...
  errorText: { color: 'red', fontSize: 16, textAlign: 'center', marginTop: 10 },
    ...
    btnM: { backgroundColor: '#fff', padding: 10, borderRadius: 5, margin: 16, width: 120, alignItems: 'center' },
  
    // Custom button styles
    btnC: { backgroundColor: '#d4edda', padding: 15, borderRadius: 8, margin: 10, width: "95%", alignItems: 'center' },
    btn1: { backgroundColor: '#00796b', padding: 15, borderRadius: 8, margin: 10, width: "95%", alignItems: 'center' },
